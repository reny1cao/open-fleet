...
export class ClaudeAgentAdapter implements AgentAdapter {
  ...
  async start(ctx: StartAgentContext): Promise<void> {
    ...
    // Try to load cached bot IDs for fast restart
    ...
    // Only validate tokens we don't have cached
    if (needsValidation.length > 0) {
      if (!opts.json && needsValidation.length < entries.length) {
        console.log(`  Validating ${needsValidation.length} new token(s) (${entries.length - needsValidation.length} cached)`)
      }
    ...
    // Inject FLEET_API_URL so task commands route via HTTP (all agents, not just remote)
    ...
    const sleepMs = agentDef.server === "local" ? 1500 : 3000
